...
#include <limits>
#include "apps/mvIPConfigure/ok_icon.xpm"
#include <string>
#include "apps/mvIPConfigure/TLILibImports.h"
...
#undef min // otherwise we can't work with the numeric_limit template here as someone might define a macro 'min'
#undef max // otherwise we can't work with the numeric_limit template here as someone might define a macro 'max'
...
//-----------------------------------------------------------------------------
{
    ...
    for( unsigned int i = 0; i < itDev->second->interfaceCount_; i++ )
    {
        if( itDev->second->interfaceInfo_[i].supportsDHCP_ )
        {
            ...
            {
                const wxString ipAddress( ConvertedString( m_interfaceInfo[i].persistentIPAddress_ ) );
                const wxString subnetMask( ConvertedString( m_interfaceInfo[i].persistentSubnetMask_ ) );
                const wxString gateway( ConvertedString( m_interfaceInfo[i].persistentDefaultGateway_ ) );
                if( !ValidateIPDataSet( ipAddress, subnetMask, gateway, this, this ) )
                {
                    return;
                }
                // In case a persistent IP will be entered , for a camera on other subnet, without a temporary ip first
                if ( m_boMarkIPAddressConflict && m_pCBUsePersistentIP->GetValue() )
                {
                    int deviceSubnet = inet_addr( itDev->second->interfaceInfo_[i].currentIPAddress_.c_str() ) &
                                       inet_addr( itDev->second->interfaceInfo_[i].currentSubnetMask_.c_str() ) ;
                    int adapterSubnet = inet_addr( itDev->second->adapters_[i].first.c_str() ) &
                                        inet_addr( itDev->second->adapters_[i].second.netMask_.c_str() );
                    int targetSubnet = inet_addr( ipAddress.mb_str() ) & inet_addr( subnetMask.mb_str() ) ;

                    // In case the camera, the adapter AND the target subnets are different, the temporary ip dialogue has to be called.
                    while( ( deviceSubnet != adapterSubnet ) && ( targetSubnet != adapterSubnet ) )
                    {
                        AssignTemporaryIP( m_pDevListCtrl->GetCurrentItemIndex(), true );
                        if( !FindDeviceWithSerial( serial, connectedToIPAddress, itInterface, itDev ) )
                        {
                            return;
                        }
                        ...
                        deviceSubnet = inet_addr( itDev->second->interfaceInfo_[i].currentIPAddress_.c_str() ) &
                                       inet_addr( itDev->second->interfaceInfo_[i].currentSubnetMask_.c_str() ) ;
                        adapterSubnet = inet_addr( itDev->second->adapters_[i].first.c_str() ) &
                                        inet_addr( itDev->second->adapters_[i].second.netMask_.c_str() );
                        if( ( deviceSubnet != adapterSubnet ) && ( targetSubnet != adapterSubnet ) )
                        {
                            WriteLogMessage( wxString::Format( wxT( "ERROR: This is no valid Temporary IP. Please try again.\n" ) ), m_ERROR_STYLE );
                            wxString errorCaption( wxT( "Not a valid temporary IP!" ) );

                            const wxString adapterSubnetString = wxString::Format( wxT( "%d.%d.%d.%d" ), ( adapterSubnet & 0xFF ),
                                                                 ( ( adapterSubnet >> 8 ) & 0xFF ), ( ( adapterSubnet >> 16 ) & 0xFF ), ( ( adapterSubnet >> 24 ) & 0xFF ) );
                            wxMessageBox( wxString::Format( wxT( "Please select an IP address in the same subnet as the adapter!\nIn this case, it is the subnet %s." ),
                                                            adapterSubnetString.c_str() ), errorCaption, wxOK | wxICON_EXCLAMATION, this );
                        }
                    }
            ...
            else if( m_interfaceInfo[i].DHCPEnabled_ )
            {
                // In case DHCP will be selected , for a camera on other subnet, without a temporary ip first
                if( m_boMarkIPAddressConflict && m_pCBUseDHCP->GetValue() && !m_pCBUsePersistentIP->GetValue() )
                {
                    AssignTemporaryIP( m_pDevListCtrl->GetCurrentItemIndex(), true );
                    if( !FindDeviceWithSerial( serial, connectedToIPAddress, itInterface, itDev ) )
                    {
...
//-----------------------------------------------------------------------------
{
    ...
    if( result != 0 )
    {
        WriteLogMessage( wxString::Format( wxT( "ERROR during call to TLIMV_IFGetDeviceInterfaceInfo( %p, %s, %d, %d, 0, %p, %p ): %d.\n" ), hInterface, deviceName.c_str(), interfaceIndex, info, pStringBuffer.get(), reinterpret_cast<void*>( &stringSize ), result ), m_ERROR_STYLE );
        return string( "" );
    }
...
//-----------------------------------------------------------------------------
{
    ...
    if( result != 0 )
    {
        WriteLogMessage( wxString::Format( wxT( "ERROR during call to IFGetDeviceInfo( %p, %s, %d, 0, 0, %p ): %d.\n" ), hInterface, deviceName.c_str(), info, reinterpret_cast<void*>( &stringSize ), result ), m_ERROR_STYLE );
        return string( "" );
    }
    auto_array_ptr<char> pStringBuffer( stringSize );
    ...
    if( result != 0 )
    {
        WriteLogMessage( wxString::Format( wxT( "ERROR during call to IFGetDeviceInfo( %p, %s, %d, 0, %p, %p ): %d.\n" ), hInterface, deviceName.c_str(), info, pStringBuffer.get(), reinterpret_cast<void*>( &stringSize ), result ), m_ERROR_STYLE );
        return string( "" );
    }
    return string( pStringBuffer.get() );
}
...
//-----------------------------------------------------------------------------
{
    ...
    if( boDeviceValid )
    {
        ...
        if( interfaceIndex == 0 )
        {
            ...
            m_boMarkIPAddressConflict = m_pTLIMV_DoAdressesMatch ? ( m_pTLIMV_DoAdressesMatch( m_pCBConnectedToIPAddress->GetValue().mb_str(), m_pSTConnectedToNetmask->GetLabel().mb_str(), it->second->interfaceInfo_[0].currentIPAddress_.c_str(), it->second->interfaceInfo_[0].currentSubnetMask_.c_str() ) != 0 ) : true;
        }
    }
    else
    {
...
//-----------------------------------------------------------------------------
void IPConfigureFrame::UpdateDeviceList( bool boBuildList /* = true*/ )
//-----------------------------------------------------------------------------
{
